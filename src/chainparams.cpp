...
public:
    CMainParams() {
        strNetworkID = "krypton";
        consensus.signet_blocks = false;
        consensus.signet_challenge.clear();
        consensus.nSubsidyHalvingInterval = 525600; // Non utilisé, car réduction personnalisée
        consensus.BIP16Height = 218579; // Hérité de Litecoin
        ...
        consensus.nPowTargetTimespan = 24 * 60 * 60; // 1 jour pour ajustement difficulté
        consensus.nPowTargetSpacing = 1 * 60; // 1 minute par bloc
        consensus.fPowAllowMinDifficultyBlocks = false;
        consensus.fPowNoRetargeting = false;
        consensus.nRuleChangeActivationThreshold = 1512; // 75% de 2016
        consensus.nMinerConfirmationWindow = 2016; // nPowTargetTimespan / nPowTargetSpacing
        consensus.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].bit = 28;
        consensus.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].nStartTime = Consensus::BIP9Deployment::NEVER_ACTIVE;
        consensus.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].nTimeout = Consensus::BIP9Deployment::NO_TIMEOUT;
        consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].bit = 2;
        consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].nStartHeight = 0; // Actif dès le départ
        consensus.vDeployments[Consensus::DEPLOYMENT_TAPROOT].nTimeoutHeight = 99999999;
        consensus.vDeployments[Consensus::DEPLOYMENT_MWEB].bit = 4;
        consensus.vDeployments[Consensus::DEPLOYMENT_MWEB].nStartHeight = 0; // Actif dès le départ
        consensus.vDeployments[Consensus::DEPLOYMENT_MWEB].nTimeoutHeight = 99999999;
        consensus.nMinimumChainWork = uint256S("0x00"); // Nouveau réseau
        consensus.defaultAssumeValid = uint256S("0x00"); // Nouveau réseau
        ...
        genesis = CreateGenesisBlock(1729440000, 2084524493, 0x1e0ffff0, 1, 1000000 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("0x...")); // Remplacer par ton hash
        assert(genesis.hashMerkleRoot == uint256S("0x...")); // Remplacer par ton hash
        vSeeds.emplace_back("ton-vps-ip:9334"); // Remplacer par l’IP de ton VPS
        ...
        bech32_hrp = "kyp";
        mweb_hrp = "kypmweb";
        vFixedSeeds.clear(); // Nouveau réseau
        fDefaultConsistencyChecks = false;
        fRequireStandard = true;
        m_is_test_chain = false;
        m_is_mockable_chain = false;
        checkpointData = {}; // Nouveau réseau, pas de checkpoints initiaux
    }
private:
    CBlock CreateGenesisBlock(uint32_t nTime, uint32_t nNonce, uint32_t nBits, int32_t nVersion, const CAmount& genesisReward)
    {
        const char* pszTimestamp = "Krypton Blockchain Launched - 20 October 2025";
        const CScript genesisOutputScript = CScript() << ParseHex("04yourpubkeyhere") << OP_CHECKSIG; // Remplacer par ta clé publique
        CTransactionRef genesisTx = MakeTransactionRef(CTransaction());
        CMutableTransaction mtx = *genesisTx;
        mtx.vout.push_back(CTxOut(1000000 * COIN, genesisOutputScript)); // 1M KYP
        genesisTx = MakeTransactionRef(mtx);
        CBlock genesis;
        genesis.nTime    = nTime;
        genesis.nBits    = nBits;
        genesis.nNonce   = nNonce;
        genesis.nVersion = nVersion;
        genesis.vtx.push_back(genesisTx);
        genesis.hashPrevBlock.SetNull();
...
public:
    CTestNetParams() {
        ...
        consensus.signet_blocks = false;
        consensus.signet_challenge.clear();
        consensus.nSubsidyHalvingInterval = 525600; // Non utilisé
        ...
        consensus.powLimit = uint256S("00000fffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        consensus.nPowTargetTimespan = 24 * 60 * 60; // 1 jour
...
private:
    CBlock CreateGenesisBlock(uint32_t nTime, uint32_t nNonce, uint32_t nBits, int32_t nVersion, const CAmount& genesisReward)
    {
        const char* pszTimestamp = "Krypton Testnet Launched - 20 October 2025";
        const CScript genesisOutputScript = CScript() << ParseHex("04yourpubkeyhere") << OP_CHECKSIG; // Remplacer par ta clé publique
        CTransactionRef genesisTx = MakeTransactionRef(CTransaction());
        CMutableTransaction mtx = *genesisTx;
        mtx.vout.push_back(CTxOut(1000000 * COIN, genesisOutputScript)); // 1M KYP
